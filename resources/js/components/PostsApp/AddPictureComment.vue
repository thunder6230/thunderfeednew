<template>
    <form ref="form" enctype="multipart/form-data" @submit.prevent="submit" class="relative">
        <input type="text" name="body" :id="`body${props.pictureId}`" class="w-full border border-blue-200 rounded-xl px-4 py-2" placeholder="What's in you mind?" ref="body">

        <label :for="`file${props.pictureId}`"
        ...
        <input type="file" name="file" :id="`file${props.pictureId}`" class="hidden" @change="handleFileInput">
...
export default {
    ...
    methods:{
        submit(){
            ...
            axios.post(`/api/pictures/${this.props.pictureId}/comment`, formData)
            .then(resp => {
